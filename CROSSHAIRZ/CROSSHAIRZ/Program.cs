...
namespace CROSSHAIRZ
{
	...
	public class CrosshairOverlay : Overlay
	{
		...
		{
			// Automatic main screen detection
		...
		{
			// --- NVIDIA FIX : GHOST MODE (Executes on the first frame) ---
			if (ImGui.GetFrameCount() == 1)
			{
				...
				// Force to foreground based on detected resolution
				...
				if (ImGui.Begin("Crosshair Editor | made by xkeishii", ref _showMenu, ImGuiWindowFlags.NoCollapse))
				{
					if (ImGui.BeginTabBar("Tabs"))
					{
						if (ImGui.BeginTabItem("Crosshair"))
						{
							...
							// FIX CS0206: Use local variables to avoid property reference errors
						...
						if (ImGui.BeginTabItem("Menu Settings"))
						{
							...
							ImGui.TextDisabled($"Detected Resolution: {_screenWidth}x{_screenHeight}");
							ImGui.TextDisabled("Press INSERT to hide/show the menu");
							ImGui.EndTabItem();
						}
						ImGui.EndTabBar();
					}
		...
		{
			// Uses automatic center instead of hardcoded 2560/1440
	...
	class Program
	{
		...
		static void Main()
		{
			// Essential so that center calculation is exact regardless of Windows scaling
			SetProcessDPIAware();
